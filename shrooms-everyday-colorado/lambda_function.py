...
def format_tweet_text(obs_formatted: dict):
    ...
    tweet = f"""The Colorado Mushroom of the {timeframe.capitalize()} is this {obs_formatted['taxon_name']}{common_name}, observed by iNat user @/{obs_formatted['inat_username']} on {obs_formatted['date_observed']} in {obs_formatted['place_guess']}. See the full observation here: {obs_formatted['inat_uri']}"""

    # todo if {obs_formatted['taxon_name']} == {state_mushroom}: tweet += "This is our official State Mushroom!"
    ...
    dt_now_pt = datetime.datetime.now(pytz.timezone('US/Mountain'))
...
def query_ddb(obs_inat_uuid: str):
    ddb_tablename = "shrooms-everyday-colorado-tweeted-observations"
